...
\begin{enumerate}
\item Dopo la registrazione (\ref{SD:1}, \ref{UC:1}), l'utente potrà usare le credenziali scelte per autenticarsi cliccando sul tasto ``Autenticati''.
...
\item A prescindere dallo stato dell'autenticazione (\ref{SD:1}, \ref{UC:1}, all'utente sarà permesso ricercare articoli nel sito. Usando la barra di ricerca presente nella pagina principale, potrà inserire dei termini da ricercare.
\item Una volta premuto sul tasto ``Cerca'', gli verrà presentato un elenco di articoli corrispondenti.

\subsubsection{Visualizzazione dettagli di un articolo}
\label{SD:4}
...
\begin{enumerate}
\item Dopo aver effettuato una ricerca (\ref{SD:3}), l'utente potrà scegliere uno degli articoli e cliccare sul nome o sull'immagine per selezionarlo.
\item All'utente verrà quindi presentata una pagina con i dettagli dell'articolo scelto.
\end{enumerate}

\subsubsection{Aggiunta articolo al carrello}
\label{SD:5}
...
\begin{enumerate}
\item Dopo aver effettuato una ricerca (\ref{SD:3}) e aver selezionato un articolo (\ref{SD:4}), un utente autenticato potrà aggiungere un articolo al carrello utilizzando il tasto dedicato.
...
\subsubsection{Acquisto articoli nel carrello}
\label{SD:6}
...
\begin{enumerate}
\item Dopo aver aggiunto almeno un articolo al carrello (\ref{SD:5}), l'utente avrà la possibilità di ordinare gli articoli nel carrello stesso.
...
\begin{enumerate}
\item All'utente sarà concessa la possibilità di vendere articoli sulla piattaforma. Dall'elenco delle vendite (\ref{SD:8}), sarà sufficiente utilizzare il tasto ``Inserisci nuova vendita".
...
\begin{enumerate}
\item Dopo aver inserito una vendita (\ref{SD:9}) e dopo che è stata accettata ( %TODO REF %, all'utente sarà permesso di modificare i dettagli della vendita.
\item Dall'elenco delle vendite, se lo stato è ancora ``In vendita", sarà presente un tasto ``Modifica vendita".
\item Se l'utente lo utilizza, verrà rimandato ad un modulo per la modifica della vendita. La vendita tornerà quindi in uno stato di attesa finché un centralinista non verificherà i dati inseriti % TODO REF
...
\begin{enumerate}
\item Dopo aver creato una vendita (\ref{SD:9}) e finché lo stato è ``In vendita", all'utente sarà concessa la possibilità di annullare una vendita.
\item Dall'elenco delle vendite sarà presente quindi un tasto ``Annulla vendita".
\end{enumerate}

% TODO stati ticket cliente

\subsection{Centralinista}

% TODO gestione ticket
% TODO gestione vendite
